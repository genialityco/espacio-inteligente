...
function ScoreGauge({ score }) {
  ...
  return (
    <svg viewBox="0 0 240 140" className="score-gauge">
      ...
      />
      <text x={cx} y={cy - 8} textAnchor="middle" fill="white" fontSize="40" fontWeight="800" fontFamily="Inter, sans-serif">{score}</text>
      <text x={cx} y={cy + 18} textAnchor="middle" fill="#94a3b8" fontSize="14" fontWeight="500" fontFamily="Inter, sans-serif">/ 100</text>
    </svg>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    if (view !== 'playing' || feedback !== null) return;
    const interval = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 0.1) { handleTimeout(); return 0; }
        return prev - 0.1;
      });
    }, 100);
  ...
  const handleTimeout = () => {
    const bad = currentRound.opciones.find(o => o.tipo === 'incorrecta') ?? currentRound.opciones[2];
    handleOptionSelect(bad);
  };

  const handleOptionSelect = (option) => {
    if (option.componentes?.length > 0) {
      setSelectedComponents(prev => [...new Set([...prev, ...option.componentes])]);
    }
    const fbType = option.tipo === 'suboptima' ? 'warning'
                 : option.tipo === 'incorrecta' ? 'negative'
                 : 'positive';
  ...
  if (view === 'idle') {
    ...
    content = (
      <SectorSelectScreen onSelect={(sectorId) => { setSector(sectorId); setView('onboarding'); }} />
    );

  } else if (view === 'onboarding') {
    const si = SECTORES[sector];
    content = (
      <div className="app-container fullscreen-view">
        <h1 className="title-xl">Tu Misión</h1>
        <div className="sector-badge">{si.emoji} {si.nombre}</div>
        <div className="onboarding-reto" style={{ borderLeftColor: si.color }}>
          <p className="reto-headline">{si.reto_central}</p>
        </div>
        ...
        <button
          ...
          onClick={() => { setView('playing'); setTimeLeft(15.0); }}
        >
          COMENZAR SIMULACIÓN
        </button>
      </div>
    ...
    content = (
      <div className="app-container fullscreen-view wow-view">
        <h1>ROI Tecnológico<br/>Maximizado</h1>
        <p className="subtitle" style={{ color: 'rgba(255,255,255,0.85)', marginTop: '1.5rem' }}>
          Podrías reducir hasta 30% en costos operativos<br/>
          y evitar el 90% de interrupciones.
        </p>
    ...
    const si = SECTORES[sector];
    const accentColor = engineResult.color === 'rojo' ? 'var(--accent-red)' : 'var(--accent-green)';
    content = (
        ...
        <div className="left-panel">
          <div className="panel-top-bar">
            <div className="round-progress">
              {rondas.map((_, i) => (
                <span key={i} className={`round-dot${i < roundIdx ? ' done' : i === roundIdx ? ' active' : ''}`} />
              ))}
            </div>
